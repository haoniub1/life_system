<template>
  <div class="task-manager">
    ...
    <!-- Native drag tasks list -->
    <div v-if="filteredTasks.length > 0" class="tasks-list" ref="taskListRef">
      <div
        ...
        :style="draggingTaskId === task.id ? dragStyle : {}"
        @mousedown="task.status === 'active' && !isDragging && startComplete(task.id, $event)"
        @mouseup="cancelComplete"
        @mouseleave="cancelComplete"
        @touchstart="task.status === 'active' && handleTouchStart(task.id, index, $event)"
      ...
      >
        ...
        <div
          ...
          class="drag-handle"
          @mousedown.stop="startDrag(task.id, index, $event)"
          @touchstart.stop="handleDragHandleTouch(task.id, index, $event)"
        >
          <span>&#x2630;</span>
        </div>
...
const dragStyle = ref<any>({})
const dragStartTimer = ref<number | null>(null)
const DRAG_START_DELAY = 300 // 300ms 长按启动拖拽
...
const handleDragHandleTouch = (taskId: number, index: number, e: TouchEvent) => {
  e.preventDefault()
...
const handleTouchMove = (e: TouchEvent) => {
  ...
  const touch = e.touches[0]
  currentTouchY.value = touch.clientY
  const deltaY = currentTouchY.value - touchStartY.value
  ...
  // 计算目标位置
  const itemHeight = 80 // 大约的任务项高度
  const newIndex = Math.max(0, Math.min(taskStore.tasks.length - 1, draggingIndex.value + Math.round(deltaY / itemHeight)))
  
  if (newIndex !== draggingIndex.value) {
    // 交换位置
    const tasks = [...taskStore.tasks]
    const draggedTask = tasks[draggingIndex.value]
    tasks.splice(draggingIndex.value, 1)
    tasks.splice(newIndex, 0, draggedTask)
    taskStore.tasks = tasks
    draggingIndex.value = newIndex
    touchStartY.value = currentTouchY.value
  }
}
...
const startDrag = (taskId: number, index: number, e: MouseEvent) => {
  // 桌面端拖拽（暂不实现，可以后续添加）
}
...
const handleTouchStart = (taskId: number, index: number, event: TouchEvent) => {
  if (isDragging.value) return
  
  // 启动完成计时
  startComplete(taskId, event)
}
